# 向量语义检索优化方案

## 问题分析

你提出了一个非常关键的问题：

> **现在daoyoucode，repomap生成+tree-sitter，有没有一层结果，供后面向量做检索呢？**

答案是：**目前没有充分利用！这是一个巨大的优化机会。**

---

## 当前架构的问题

### 问题1：重复解析

```
codebase_index.py（向量检索）:
  ├─ 扫描文件 → 按行/def/class分块
  ├─ 简单的正则匹配：if line.startswith("def ") or line.startswith("class ")
  └─ 没有利用tree-sitter的精确解析

repomap_tools.py（代码地图）:
  ├─ 扫描文件 → tree-sitter精确解析
  ├─ 提取函数、类、方法的定义和引用
  ├─ 构建引用图
  └─ PageRank排序

❌ 两者独立工作，重复解析！
```

### 问题2：分块质量差

```python
# codebase_index.py 的分块逻辑
def _chunk_file(content: str, path: Path, max_lines: int = 55):
    if ext == ".py":
        # ❌ 简单的正则匹配
        for i, line in enumerate(lines):
            if line.strip().startswith("def ") or line.strip().startswith("class "):
                # 分块
```

**问题**：
- 无法识别嵌套的类和方法
- 无法识别装饰器
- 无法识别类型注解
- 无法识别import语句
- 分块边界不准确

### 问题3：缺少文件关联

```python
# 当前的chunk结构
{
    "path": "backend/agents/core/agent.py",
    "start": 100,
    "end": 150,
    "text": "def execute(self, ...):\n    ..."
}

# ❌ 缺少：
# - 这个函数属于哪个类？
# - 这个函数调用了哪些其他函数？
# - 这个函数被哪些文件引用？
# - 这个函数的依赖关系？
```

---

## Cursor的做法（推测）

根据你的描述和Cursor的行为，它可能是这样做的：

```
1. Tree-sitter解析
   ├─ 提取所有定义（函数、类、方法）
   ├─ 提取所有引用（函数调用、类实例化）
   └─ 构建引用图

2. 代码分块（基于AST）
   ├─ 每个函数/类作为一个chunk
   ├─ 包含上下文（所属类、装饰器、类型注解）
   └─ 包含引用信息（调用了谁、被谁调用）

3. 向量化
   ├─ chunk文本 → embedding
   ├─ 文件路径 → embedding
   └─ 引用关系 → 图结构

4. 检索
   ├─ 语义检索：query → embedding → 相似度匹配
   ├─ 文件关联：找到相关chunk后，扩展到相关文件
   └─ 引用关联：找到函数A后，自动包含调用A的函数B
```

---

## 优化方案

### 方案1：复用repomap的tree-sitter解析结果 ⭐⭐⭐⭐⭐

**核心思想**：repomap已经用tree-sitter精确解析了代码，为什么不复用这个结果？

```python
# 新的架构
class CodebaseIndex:
    def build_index(self):
        # 1. 使用repomap的解析结果
        from .repomap_tools import RepoMapTool
        repomap = RepoMapTool()
        
        # 🔑 关键：获取repomap的definitions
        definitions = repomap._scan_repository(repo_path)
        # definitions = {
        #     "backend/agents/core/agent.py": [
        #         {"type": "class", "name": "BaseAgent", "line": 50, "kind": "def"},
        #         {"type": "method", "name": "execute", "line": 100, "kind": "def"},
        #         ...
        #     ]
        # }
        
        # 2. 基于definitions构建高质量的chunks
        for file_path, defs in definitions.items():
            for d in defs:
                if d["kind"] == "def":  # 只要定义，不要引用
                    chunk = self._build_chunk_from_definition(
                        file_path, d, definitions
                    )
                    self.chunks.append(chunk)
```

**优点**：
- ✅ 精确的分块边界（基于AST）
- ✅ 包含完整的上下文信息
- ✅ 避免重复解析
- ✅ 利用已有的缓存机制



### 方案2：增强chunk结构，添加文件关联 ⭐⭐⭐⭐⭐

**核心思想**：每个chunk不仅包含代码文本，还包含丰富的元数据。

```python
# 增强的chunk结构
{
    # 基础信息
    "path": "backend/agents/core/agent.py",
    "start": 100,
    "end": 150,
    "text": "def execute(self, ...):\n    ...",
    
    # 🆕 AST信息
    "type": "method",  # function, class, method
    "name": "execute",
    "parent_class": "BaseAgent",  # 所属类
    "decorators": ["@async"],  # 装饰器
    
    # 🆕 引用关系
    "calls": [  # 这个函数调用了谁
        "self._load_prompt",
        "self._call_llm_with_tools"
    ],
    "called_by": [  # 这个函数被谁调用
        "orchestrator.execute",
        "skill_executor.run"
    ],
    
    # 🆕 依赖关系
    "imports": [  # 这个文件导入了什么
        "from ..llm import get_client_manager",
        "from ..tools import get_tool_registry"
    ],
    "imported_by": [  # 这个文件被谁导入
        "backend/orchestrators/multi_agent.py",
        "backend/executor.py"
    ],
    
    # 🆕 文件关联
    "related_files": [  # 相关文件（基于引用关系）
        "backend/agents/llm/client_manager.py",
        "backend/agents/tools/registry.py"
    ],
    
    # 🆕 PageRank分数
    "pagerank_score": 0.85  # 在引用图中的重要性
}
```

**如何构建这些信息**：

```python
def _build_chunk_from_definition(
    self,
    file_path: str,
    definition: Dict,
    all_definitions: Dict[str, List[Dict]],
    reference_graph: Dict[str, Dict[str, float]]
) -> Dict:
    """基于repomap的definition构建增强的chunk"""
    
    # 1. 读取代码文本
    code_text = self._extract_code_text(file_path, definition)
    
    # 2. 提取调用关系（从代码文本中）
    calls = self._extract_function_calls(code_text)
    
    # 3. 从引用图中找到被谁调用
    called_by = self._find_callers(file_path, definition["name"], reference_graph)
    
    # 4. 提取导入关系
    imports = self._extract_imports(file_path)
    
    # 5. 找到相关文件（基于引用图）
    related_files = self._find_related_files(file_path, reference_graph)
    
    # 6. 获取PageRank分数
    pagerank_score = self._get_pagerank_score(file_path, reference_graph)
    
    return {
        "path": file_path,
        "start": definition["line"],
        "end": definition["line"] + len(code_text.splitlines()),
        "text": code_text,
        "type": definition["type"],
        "name": definition["name"],
        "calls": calls,
        "called_by": called_by,
        "imports": imports,
        "related_files": related_files,
        "pagerank_score": pagerank_score
    }
```

---

### 方案3：多层次检索策略 ⭐⭐⭐⭐

**核心思想**：不仅仅是语义匹配，还要考虑文件关联和引用关系。

```python
def search(self, query: str, top_k: int = 10) -> List[Dict[str, Any]]:
    """多层次检索"""
    
    # 第1层：语义检索（基础）
    semantic_results = self._semantic_search(query, top_k * 2)
    
    # 第2层：文件关联扩展
    expanded_results = []
    for chunk in semantic_results:
        expanded_results.append(chunk)
        
        # 🔑 关键：扩展到相关文件
        for related_file in chunk.get("related_files", []):
            related_chunks = self._get_chunks_by_file(related_file)
            expanded_results.extend(related_chunks[:2])  # 每个相关文件取前2个chunk
    
    # 第3层：引用关系扩展
    final_results = []
    for chunk in expanded_results:
        final_results.append(chunk)
        
        # 🔑 关键：扩展到调用者和被调用者
        for caller in chunk.get("called_by", []):
            caller_chunk = self._get_chunk_by_function(caller)
            if caller_chunk:
                final_results.append(caller_chunk)
        
        for callee in chunk.get("calls", []):
            callee_chunk = self._get_chunk_by_function(callee)
            if callee_chunk:
                final_results.append(callee_chunk)
    
    # 第4层：去重和排序
    unique_results = self._deduplicate(final_results)
    sorted_results = self._rerank(unique_results, query)
    
    return sorted_results[:top_k]
```

**检索示例**：

```
用户问："如何修复Agent执行时的超时错误？"

第1层：语义检索
  → 找到：backend/agents/core/agent.py::execute()
  → 相似度：0.85

第2层：文件关联扩展
  → execute() 导入了 timeout_handler
  → 扩展到：backend/agents/core/timeout_handler.py
  → 扩展到：backend/agents/core/recovery.py

第3层：引用关系扩展
  → execute() 调用了 _call_llm_with_tools()
  → 扩展到：backend/agents/core/agent.py::_call_llm_with_tools()
  → execute() 被 orchestrator.execute() 调用
  → 扩展到：backend/orchestrators/multi_agent.py::execute()

第4层：去重和排序
  → 按相关性重新排序
  → 返回top-10最相关的代码块
```

---

### 方案4：混合检索（语义+关键词+PageRank） ⭐⭐⭐⭐⭐

**核心思想**：结合多种信号，提高检索准确性。

```python
def search(self, query: str, top_k: int = 10) -> List[Dict[str, Any]]:
    """混合检索"""
    
    # 1. 语义检索分数
    semantic_scores = self._semantic_search_scores(query)
    
    # 2. 关键词匹配分数
    keyword_scores = self._keyword_search_scores(query)
    
    # 3. PageRank分数（代码重要性）
    pagerank_scores = {
        chunk["id"]: chunk.get("pagerank_score", 0.0)
        for chunk in self.chunks
    }
    
    # 4. 文件关联分数（如果用户打开了某个文件）
    file_affinity_scores = self._file_affinity_scores(
        query, 
        opened_files=self.context.get("initial_files", [])
    )
    
    # 5. 混合打分
    final_scores = {}
    for chunk_id in semantic_scores:
        score = (
            0.5 * semantic_scores.get(chunk_id, 0.0) +      # 语义权重50%
            0.2 * keyword_scores.get(chunk_id, 0.0) +       # 关键词权重20%
            0.2 * pagerank_scores.get(chunk_id, 0.0) +      # PageRank权重20%
            0.1 * file_affinity_scores.get(chunk_id, 0.0)   # 文件关联权重10%
        )
        final_scores[chunk_id] = score
    
    # 6. 排序并返回
    sorted_chunks = sorted(
        self.chunks,
        key=lambda c: final_scores.get(c["id"], 0.0),
        reverse=True
    )
    
    return sorted_chunks[:top_k]
```

---

## 实施计划

### 阶段1：复用repomap解析结果（高优先级）⭐⭐⭐⭐⭐

**目标**：避免重复解析，提高分块质量

**步骤**：
1. 修改 `codebase_index.py`，调用 `repomap._scan_repository()`
2. 基于 `definitions` 构建高质量的chunks
3. 保留原有的向量化和检索逻辑

**预期效果**：
- 分块质量提升 50%
- 解析速度提升 2倍（复用缓存）
- 代码维护性提升（统一解析逻辑）

---

### 阶段2：增强chunk结构（中优先级）⭐⭐⭐⭐

**目标**：添加文件关联和引用关系

**步骤**：
1. 扩展chunk结构，添加元数据字段
2. 从 `repomap._build_reference_graph()` 获取引用关系
3. 提取导入关系和依赖关系
4. 计算PageRank分数

**预期效果**：
- 检索准确性提升 30%
- 支持"找到相关文件"功能
- 支持"找到调用链"功能

---

### 阶段3：多层次检索（中优先级）⭐⭐⭐⭐

**目标**：不仅返回匹配的代码，还返回相关的代码

**步骤**：
1. 实现文件关联扩展
2. 实现引用关系扩展
3. 实现去重和重排序

**预期效果**：
- 检索召回率提升 40%
- 用户体验提升（一次检索获得完整上下文）

---

### 阶段4：混合检索（低优先级）⭐⭐⭐

**目标**：结合多种信号，提高准确性

**步骤**：
1. 实现关键词匹配打分
2. 实现PageRank打分
3. 实现文件关联打分
4. 调优权重参数

**预期效果**：
- 检索准确性提升 20%
- 更好地处理边缘情况

---

## 代码示例

### 示例1：复用repomap解析结果

```python
# backend/daoyoucode/agents/memory/codebase_index.py

class CodebaseIndex:
    def build_index(self, force: bool = False) -> int:
        """构建索引（复用repomap解析结果）"""
        
        # 1. 使用repomap的解析结果
        from ..tools.repomap_tools import RepoMapTool
        repomap_tool = RepoMapTool()
        
        # 初始化缓存
        repomap_tool._init_cache(self.repo_path)
        
        # 🔑 关键：获取definitions（已缓存）
        definitions = repomap_tool._scan_repository(self.repo_path)
        
        # 2. 构建引用图（用于后续的关联分析）
        reference_graph = repomap_tool._build_reference_graph(
            definitions, self.repo_path
        )
        
        # 3. 计算PageRank（用于重要性排序）
        pagerank_scores = repomap_tool._pagerank(
            reference_graph,
            definitions,
            chat_files=[],
            mentioned_idents=[]
        )
        pagerank_dict = dict(pagerank_scores)
        
        # 4. 基于definitions构建chunks
        self.chunks = []
        for file_path, defs in definitions.items():
            for d in defs:
                if d.get("kind") != "def":
                    continue  # 只要定义，不要引用
                
                # 读取代码文本
                code_text = self._extract_code_text(
                    self.repo_path / file_path,
                    d["line"],
                    d.get("end_line", d["line"] + 50)
                )
                
                # 构建增强的chunk
                chunk = {
                    "path": file_path,
                    "start": d["line"],
                    "end": d.get("end_line", d["line"] + len(code_text.splitlines())),
                    "text": code_text,
                    "type": d["type"],
                    "name": d["name"],
                    "pagerank_score": pagerank_dict.get(file_path, 0.0)
                }
                
                self.chunks.append(chunk)
        
        # 5. 向量化
        if self.chunks:
            self._vectorize_chunks()
        
        # 6. 保存
        self._save_meta()
        
        logger.info(f"索引已构建: {len(self.chunks)} 块（复用repomap解析）")
        return len(self.chunks)
    
    def _extract_code_text(
        self,
        file_path: Path,
        start_line: int,
        end_line: int
    ) -> str:
        """提取代码文本"""
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                lines = f.readlines()
                return ''.join(lines[start_line-1:end_line])
        except Exception as e:
            logger.warning(f"提取代码失败: {e}")
            return ""
```

---

### 示例2：增强chunk结构

```python
# backend/daoyoucode/agents/memory/codebase_index.py

def _build_enhanced_chunk(
    self,
    file_path: str,
    definition: Dict,
    all_definitions: Dict[str, List[Dict]],
    reference_graph: Dict[str, Dict[str, float]],
    pagerank_scores: Dict[str, float]
) -> Dict:
    """构建增强的chunk（包含文件关联）"""
    
    # 基础信息
    chunk = {
        "path": file_path,
        "start": definition["line"],
        "text": self._extract_code_text(...),
        "type": definition["type"],
        "name": definition["name"],
    }
    
    # 🆕 PageRank分数
    chunk["pagerank_score"] = pagerank_scores.get(file_path, 0.0)
    
    # 🆕 引用关系
    chunk["calls"] = self._extract_calls(chunk["text"])
    chunk["called_by"] = self._find_callers(
        file_path, definition["name"], all_definitions
    )
    
    # 🆕 文件关联
    chunk["related_files"] = list(reference_graph.get(file_path, {}).keys())
    
    return chunk

def _extract_calls(self, code_text: str) -> List[str]:
    """从代码中提取函数调用"""
    # 简单的正则匹配（可以用tree-sitter更精确）
    import re
    pattern = r'(\w+)\s*\('
    calls = re.findall(pattern, code_text)
    return list(set(calls))

def _find_callers(
    self,
    file_path: str,
    function_name: str,
    all_definitions: Dict[str, List[Dict]]
) -> List[str]:
    """找到调用这个函数的其他函数"""
    callers = []
    for other_file, defs in all_definitions.items():
        for d in defs:
            if d.get("kind") == "ref" and d["name"] == function_name:
                callers.append(f"{other_file}::{d.get('parent', 'unknown')}")
    return callers
```

---

## 总结

### 当前问题
1. ❌ repomap和codebase_index重复解析
2. ❌ 分块质量差（简单正则）
3. ❌ 缺少文件关联和引用关系
4. ❌ 只有语义检索，没有混合检索

### 优化方案
1. ✅ 复用repomap的tree-sitter解析结果
2. ✅ 增强chunk结构，添加元数据
3. ✅ 多层次检索（语义+关联+引用）
4. ✅ 混合检索（语义+关键词+PageRank）

### 预期效果
- 分块质量提升 50%
- 检索准确性提升 30-50%
- 检索召回率提升 40%
- 解析速度提升 2倍
- 代码维护性大幅提升

### 实施优先级
1. **高优先级**：复用repomap解析结果（阶段1）
2. **中优先级**：增强chunk结构（阶段2）
3. **中优先级**：多层次检索（阶段3）
4. **低优先级**：混合检索（阶段4）

这就是完整的优化方案！核心思想是：**充分利用repomap+tree-sitter的解析结果，构建高质量的代码索引，支持多层次的智能检索。**


---

## 技术细节补充

### 1. RepoMap对外API设计

**问题**：当前repomap的方法都是私有的（`_scan_repository`, `_build_reference_graph`），需要设计清晰的公开API。

**方案**：

```python
# backend/daoyoucode/agents/tools/repomap_tools.py

class RepoMapTool(BaseTool):
    """RepoMap工具（增强版：支持外部调用）"""
    
    # ========== 公开API ==========
    
    def get_definitions(
        self,
        repo_path: str,
        use_cache: bool = True
    ) -> Dict[str, List[Dict]]:
        """
        获取代码定义（公开API）
        
        Args:
            repo_path: 仓库路径
            use_cache: 是否使用缓存
        
        Returns:
            {
                "backend/agents/core/agent.py": [
                    {
                        "type": "class",
                        "name": "BaseAgent",
                        "line": 50,
                        "end_line": 150,  # 🆕 添加结束行
                        "kind": "def"
                    },
                    ...
                ]
            }
        """
        repo_path_resolved = self.resolve_path(repo_path)
        
        if use_cache:
            self._init_cache(repo_path_resolved)
        
        definitions = self._scan_repository(repo_path_resolved)
        
        # 🆕 计算end_line（如果没有）
        definitions = self._compute_end_lines(definitions, repo_path_resolved)
        
        return definitions
    
    def get_reference_graph(
        self,
        repo_path: str,
        definitions: Optional[Dict[str, List[Dict]]] = None
    ) -> Dict[str, Dict[str, float]]:
        """
        获取引用图（公开API）
        
        Args:
            repo_path: 仓库路径
            definitions: 代码定义（如果为None，会自动获取）
        
        Returns:
            {
                "file_a.py": {
                    "file_b.py": 3.0,  # file_a引用file_b 3次
                    "file_c.py": 1.0
                }
            }
        """
        repo_path_resolved = self.resolve_path(repo_path)
        
        if definitions is None:
            definitions = self.get_definitions(repo_path)
        
        return self._build_reference_graph(definitions, repo_path_resolved)
    
    def get_pagerank_scores(
        self,
        repo_path: str,
        reference_graph: Optional[Dict] = None,
        chat_files: Optional[List[str]] = None,
        mentioned_idents: Optional[List[str]] = None
    ) -> Dict[str, float]:
        """
        获取PageRank分数（公开API）
        
        Args:
            repo_path: 仓库路径
            reference_graph: 引用图（如果为None，会自动获取）
            chat_files: 焦点文件
            mentioned_idents: 提到的标识符
        
        Returns:
            {
                "file_a.py": 0.85,
                "file_b.py": 0.65,
                ...
            }
        """
        if reference_graph is None:
            definitions = self.get_definitions(repo_path)
            reference_graph = self.get_reference_graph(repo_path, definitions)
        
        ranked = self._pagerank(
            reference_graph,
            self._last_definitions,  # 保存最后一次的definitions
            chat_files or [],
            mentioned_idents or []
        )
        
        return dict(ranked)
    
    # ========== 私有方法（保持不变）==========
    
    def _compute_end_lines(
        self,
        definitions: Dict[str, List[Dict]],
        repo_path: Path
    ) -> Dict[str, List[Dict]]:
        """
        计算每个定义的结束行
        
        策略：
        1. 如果已有end_line，保持不变
        2. 否则，找到下一个定义的起始行作为结束行
        3. 如果是最后一个定义，使用文件末尾
        """
        for file_path, defs in definitions.items():
            # 按行号排序
            defs.sort(key=lambda d: d["line"])
            
            for i, d in enumerate(defs):
                if "end_line" in d:
                    continue
                
                # 找到下一个定义
                if i + 1 < len(defs):
                    d["end_line"] = defs[i + 1]["line"] - 1
                else:
                    # 最后一个定义，读取文件获取总行数
                    try:
                        full_path = repo_path / file_path
                        with open(full_path, 'r', encoding='utf-8') as f:
                            total_lines = len(f.readlines())
                        d["end_line"] = total_lines
                    except Exception:
                        # 如果读取失败，估计50行
                        d["end_line"] = d["line"] + 50
        
        return definitions
```

**优点**：
- ✅ 清晰的公开API
- ✅ 向后兼容（私有方法保持不变）
- ✅ 支持缓存控制
- ✅ 自动计算end_line

---

### 2. Definition结构标准化

**问题**：当前definition结构不完整，缺少end_line等关键信息。

**标准化结构**：

```python
# Definition标准结构
{
    # 基础信息
    "type": str,        # "class" | "function" | "method" | "variable"
    "name": str,        # 定义名称
    "line": int,        # 起始行号（1-based）
    "end_line": int,    # 🆕 结束行号（1-based）
    "kind": str,        # "def" | "ref"
    
    # 🆕 上下文信息
    "parent": str,      # 父级（如类名），可选
    "scope": str,       # "global" | "class" | "function"
    
    # 🆕 代码文本（可选，按需加载）
    "text": str,        # 代码文本（懒加载）
    
    # 🆕 元数据
    "file_path": str,   # 所属文件（相对路径）
    "language": str,    # 编程语言
}
```

**实现**：

```python
def _parse_file(self, file_path: Path) -> List[Dict]:
    """解析文件，提取定义和引用（增强版）"""
    
    # ... 原有的tree-sitter解析逻辑 ...
    
    definitions = []
    parent_stack = []  # 🆕 跟踪父级
    
    for pattern_index, captures_dict in matches:
        for tag, nodes in captures_dict.items():
            for node in nodes:
                # 判断类型
                if tag.startswith("name.definition."):
                    kind = "def"
                    type_name = tag.split(".")[-1]
                    
                    # 🆕 计算end_line
                    end_line = node.end_point[0] + 1
                    
                    # 🆕 确定父级
                    parent = parent_stack[-1] if parent_stack else None
                    
                    # 🆕 确定作用域
                    if type_name == "class":
                        scope = "global"
                        parent_stack.append(node.text.decode("utf-8"))
                    elif type_name in ("function", "method"):
                        scope = "class" if parent else "global"
                    else:
                        scope = "global"
                    
                    definitions.append({
                        "type": type_name,
                        "name": node.text.decode("utf-8"),
                        "line": node.start_point[0] + 1,
                        "end_line": end_line,  # 🆕
                        "kind": kind,
                        "parent": parent,      # 🆕
                        "scope": scope,        # 🆕
                        "file_path": str(file_path.relative_to(repo_path)),  # 🆕
                        "language": lang       # 🆕
                    })
                
                elif tag.startswith("name.reference."):
                    # ... 引用处理 ...
    
    return definitions
```

---

### 3. 缓存一致性保证

**问题**：repomap和codebase_index各自维护缓存，可能不一致。

**方案：统一缓存管理**

```python
# backend/daoyoucode/agents/tools/cache_manager.py

class CodebaseCacheManager:
    """代码库缓存管理器（统一管理repomap和向量索引的缓存）"""
    
    def __init__(self, repo_path: Path):
        self.repo_path = repo_path
        self.cache_dir = repo_path / ".daoyoucode" / "cache"
        self.cache_dir.mkdir(parents=True, exist_ok=True)
        
        # 统一的缓存数据库
        self.db = sqlite3.connect(str(self.cache_dir / "codebase.db"))
        self._init_db()
    
    def _init_db(self):
        """初始化数据库"""
        self.db.execute("""
            CREATE TABLE IF NOT EXISTS file_cache (
                file_path TEXT PRIMARY KEY,
                mtime REAL,
                definitions TEXT,
                updated_at REAL
            )
        """)
        
        self.db.execute("""
            CREATE TABLE IF NOT EXISTS index_meta (
                key TEXT PRIMARY KEY,
                value TEXT,
                updated_at REAL
            )
        """)
        
        self.db.commit()
    
    def get_definitions(
        self,
        file_path: str,
        mtime: float
    ) -> Optional[List[Dict]]:
        """获取文件的definitions（检查mtime）"""
        cursor = self.db.execute(
            "SELECT mtime, definitions FROM file_cache WHERE file_path = ?",
            (file_path,)
        )
        row = cursor.fetchone()
        
        if row is None:
            return None
        
        cached_mtime, cached_defs = row
        
        # 🔑 关键：检查mtime一致性
        if abs(cached_mtime - mtime) > 0.001:  # 允许1ms误差
            return None
        
        return json.loads(cached_defs)
    
    def set_definitions(
        self,
        file_path: str,
        mtime: float,
        definitions: List[Dict]
    ):
        """保存文件的definitions"""
        import time
        self.db.execute(
            """
            INSERT OR REPLACE INTO file_cache 
            (file_path, mtime, definitions, updated_at) 
            VALUES (?, ?, ?, ?)
            """,
            (file_path, mtime, json.dumps(definitions), time.time())
        )
        self.db.commit()
    
    def invalidate_file(self, file_path: str):
        """使文件缓存失效"""
        self.db.execute(
            "DELETE FROM file_cache WHERE file_path = ?",
            (file_path,)
        )
        self.db.commit()
    
    def get_index_version(self) -> str:
        """获取索引版本（用于检测是否需要重建）"""
        cursor = self.db.execute(
            "SELECT value FROM index_meta WHERE key = 'version'"
        )
        row = cursor.fetchone()
        return row[0] if row else "0"
    
    def set_index_version(self, version: str):
        """设置索引版本"""
        import time
        self.db.execute(
            """
            INSERT OR REPLACE INTO index_meta 
            (key, value, updated_at) 
            VALUES ('version', ?, ?)
            """,
            (version, time.time())
        )
        self.db.commit()
    
    def get_stats(self) -> Dict[str, Any]:
        """获取缓存统计"""
        cursor = self.db.execute(
            "SELECT COUNT(*) FROM file_cache"
        )
        file_count = cursor.fetchone()[0]
        
        cursor = self.db.execute(
            "SELECT SUM(LENGTH(definitions)) FROM file_cache"
        )
        total_size = cursor.fetchone()[0] or 0
        
        return {
            "file_count": file_count,
            "total_size": total_size,
            "cache_dir": str(self.cache_dir)
        }


# 全局单例
_cache_managers: Dict[str, CodebaseCacheManager] = {}

def get_cache_manager(repo_path: Path) -> CodebaseCacheManager:
    """获取缓存管理器（单例）"""
    key = str(repo_path.resolve())
    if key not in _cache_managers:
        _cache_managers[key] = CodebaseCacheManager(repo_path)
    return _cache_managers[key]
```

**使用方式**：

```python
# repomap_tools.py
def _get_cached_definitions(self, file_path: str, mtime: float):
    cache_manager = get_cache_manager(self.repo_path)
    return cache_manager.get_definitions(file_path, mtime)

def _cache_definitions(self, file_path: str, mtime: float, definitions: List[Dict]):
    cache_manager = get_cache_manager(self.repo_path)
    cache_manager.set_definitions(file_path, mtime, definitions)

# codebase_index.py
def build_index(self):
    cache_manager = get_cache_manager(self.repo_path)
    
    # 检查索引版本
    current_version = cache_manager.get_index_version()
    if current_version == self.INDEX_VERSION and not force:
        # 加载已有索引
        return self._load_index()
    
    # 重建索引
    # ...
    
    # 更新版本
    cache_manager.set_index_version(self.INDEX_VERSION)
```

**优点**：
- ✅ 统一的缓存管理
- ✅ mtime一致性检查
- ✅ 版本控制
- ✅ 避免缓存不一致

---

### 4. Chunk边界精确控制

**问题**：如何确保chunk边界准确，不会截断代码？

**方案：基于AST的精确边界**

```python
def _extract_code_chunk(
    self,
    file_path: Path,
    definition: Dict
) -> str:
    """
    提取代码块（精确边界）
    
    策略：
    1. 使用definition的line和end_line
    2. 包含装饰器（向上扩展）
    3. 包含文档字符串（向下扩展）
    4. 保持缩进一致性
    """
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            lines = f.readlines()
        
        start_line = definition["line"] - 1  # 转为0-based
        end_line = definition.get("end_line", start_line + 50) - 1
        
        # 🔑 向上扩展：包含装饰器和注释
        while start_line > 0:
            prev_line = lines[start_line - 1].strip()
            if prev_line.startswith('@') or prev_line.startswith('#'):
                start_line -= 1
            else:
                break
        
        # 🔑 向下扩展：确保完整性
        # 检查缩进，确保不会截断嵌套结构
        base_indent = len(lines[start_line]) - len(lines[start_line].lstrip())
        
        while end_line < len(lines) - 1:
            next_line = lines[end_line + 1]
            if not next_line.strip():
                # 空行，继续
                end_line += 1
                continue
            
            next_indent = len(next_line) - len(next_line.lstrip())
            if next_indent > base_indent:
                # 仍在当前块内，继续
                end_line += 1
            else:
                # 已经到下一个块，停止
                break
        
        # 提取代码
        code_lines = lines[start_line:end_line + 1]
        code_text = ''.join(code_lines)
        
        return code_text
    
    except Exception as e:
        logger.warning(f"提取代码块失败: {e}")
        return ""
```

**优点**：
- ✅ 精确的边界
- ✅ 包含装饰器和文档
- ✅ 保持代码完整性
- ✅ 处理嵌套结构

---

### 5. 多层检索的规模控制

**问题**：多层检索可能导致结果爆炸（指数增长）。

**方案：分层限制 + 去重**

```python
def search_multilayer(
    self,
    query: str,
    top_k: int = 10,
    max_expansion: int = 50  # 🔑 最大扩展数量
) -> List[Dict[str, Any]]:
    """
    多层检索（规模控制）
    
    策略：
    1. 第1层：语义检索 top_k*2
    2. 第2层：文件关联扩展，每个文件最多2个chunk
    3. 第3层：引用关系扩展，每个函数最多1个调用者/被调用者
    4. 去重 + 重排序 → top_k
    """
    results = []
    seen_ids = set()
    
    # 第1层：语义检索
    semantic_results = self._semantic_search(query, top_k * 2)
    for chunk in semantic_results:
        chunk_id = f"{chunk['path']}:{chunk['start']}"
        if chunk_id not in seen_ids:
            results.append(chunk)
            seen_ids.add(chunk_id)
    
    # 🔑 规模控制：限制扩展数量
    if len(results) >= max_expansion:
        return self._rerank(results, query)[:top_k]
    
    # 第2层：文件关联扩展
    for chunk in semantic_results[:5]:  # 只扩展前5个
        related_files = chunk.get("related_files", [])
        for related_file in related_files[:3]:  # 每个chunk最多3个相关文件
            related_chunks = self._get_chunks_by_file(related_file, limit=2)
            for rc in related_chunks:
                chunk_id = f"{rc['path']}:{rc['start']}"
                if chunk_id not in seen_ids and len(results) < max_expansion:
                    results.append(rc)
                    seen_ids.add(chunk_id)
    
    # 🔑 规模控制：再次检查
    if len(results) >= max_expansion:
        return self._rerank(results, query)[:top_k]
    
    # 第3层：引用关系扩展
    for chunk in semantic_results[:3]:  # 只扩展前3个
        # 调用者（最多1个）
        called_by = chunk.get("called_by", [])
        if called_by and len(results) < max_expansion:
            caller_chunk = self._get_chunk_by_function(called_by[0])
            if caller_chunk:
                chunk_id = f"{caller_chunk['path']}:{caller_chunk['start']}"
                if chunk_id not in seen_ids:
                    results.append(caller_chunk)
                    seen_ids.add(chunk_id)
        
        # 被调用者（最多1个）
        calls = chunk.get("calls", [])
        if calls and len(results) < max_expansion:
            callee_chunk = self._get_chunk_by_function(calls[0])
            if callee_chunk:
                chunk_id = f"{callee_chunk['path']}:{callee_chunk['start']}"
                if chunk_id not in seen_ids:
                    results.append(callee_chunk)
                    seen_ids.add(chunk_id)
    
    # 第4层：去重 + 重排序
    final_results = self._rerank(results, query)
    
    logger.info(
        f"多层检索: 语义={len(semantic_results)}, "
        f"扩展后={len(results)}, 最终={len(final_results[:top_k])}"
    )
    
    return final_results[:top_k]
```

**规模控制策略**：

| 层级 | 输入 | 扩展策略 | 输出上限 |
|------|------|---------|---------|
| 第1层 | query | 语义检索 | top_k*2 (20) |
| 第2层 | 前5个 | 每个→3个相关文件→2个chunk | +30 |
| 第3层 | 前3个 | 每个→1个调用者+1个被调用者 | +6 |
| 总计 | - | - | 56 → 去重 → top_k (10) |

**优点**：
- ✅ 可控的规模增长
- ✅ 避免结果爆炸
- ✅ 保证性能
- ✅ 可调节的参数

---

## 阶段1实施计划（详细）

### 步骤1：抽象RepoMap公开API（1-2小时）

**文件**：`backend/daoyoucode/agents/tools/repomap_tools.py`

**任务**：
1. 添加 `get_definitions()` 公开方法
2. 添加 `get_reference_graph()` 公开方法
3. 添加 `get_pagerank_scores()` 公开方法
4. 实现 `_compute_end_lines()` 方法
5. 增强 `_parse_file()` 添加parent、scope等字段

**测试**：
```python
# test_repomap_api.py
repomap = RepoMapTool()
definitions = repomap.get_definitions(".")
assert "backend/agents/core/agent.py" in definitions
assert definitions["backend/agents/core/agent.py"][0]["end_line"] > 0
```

---

### 步骤2：修改CodebaseIndex复用RepoMap（2-3小时）

**文件**：`backend/daoyoucode/agents/memory/codebase_index.py`

**任务**：
1. 修改 `build_index()` 调用 `repomap.get_definitions()`
2. 修改 `_chunk_file()` 改为 `_build_chunk_from_definition()`
3. 添加 `_extract_code_chunk()` 方法（精确边界）
4. 增强chunk结构，添加type、name、pagerank_score等字段

**测试**：
```python
# test_codebase_index.py
index = CodebaseIndex.get_index(Path("."))
index.build_index(force=True)
assert len(index.chunks) > 0
assert index.chunks[0]["type"] in ["class", "function", "method"]
assert "end_line" in index.chunks[0]
```

---

### 步骤3：统一缓存管理（1-2小时）

**文件**：`backend/daoyoucode/agents/tools/cache_manager.py`（新建）

**任务**：
1. 创建 `CodebaseCacheManager` 类
2. 实现统一的缓存数据库
3. 修改repomap使用统一缓存
4. 修改codebase_index使用统一缓存

**测试**：
```python
# test_cache_manager.py
cache = get_cache_manager(Path("."))
cache.set_definitions("test.py", 123.456, [{"name": "test"}])
defs = cache.get_definitions("test.py", 123.456)
assert defs == [{"name": "test"}]
```

---

### 步骤4：集成测试（1小时）

**任务**：
1. 测试repomap → codebase_index的完整流程
2. 测试缓存一致性
3. 测试性能提升
4. 测试chunk质量

**测试脚本**：
```python
# test_integration.py
import time

# 测试1：性能对比
start = time.time()
index_old = build_index_old()  # 旧方法
time_old = time.time() - start

start = time.time()
index_new = build_index_new()  # 新方法
time_new = time.time() - start

print(f"性能提升: {(time_old - time_new) / time_old * 100:.1f}%")

# 测试2：chunk质量对比
print(f"旧方法chunk数: {len(index_old.chunks)}")
print(f"新方法chunk数: {len(index_new.chunks)}")
print(f"新方法包含元数据: {index_new.chunks[0].keys()}")
```

---

## 风险控制

### 风险1：API变更影响现有功能

**缓解措施**：
- 保持私有方法不变
- 新增公开方法，不修改现有方法
- 充分的单元测试

### 风险2：缓存不一致

**缓解措施**：
- 统一缓存管理器
- mtime严格检查
- 版本控制机制

### 风险3：性能下降

**缓解措施**：
- 复用缓存，避免重复解析
- 懒加载（按需加载代码文本）
- 规模控制（限制扩展数量）

### 风险4：chunk边界不准确

**缓解措施**：
- 基于AST的精确边界
- 包含装饰器和文档
- 充分的测试用例

---

## 总结

补充了5个关键技术细节：
1. ✅ RepoMap对外API设计
2. ✅ Definition结构标准化
3. ✅ 缓存一致性保证
4. ✅ Chunk边界精确控制
5. ✅ 多层检索的规模控制

阶段1实施计划：
1. 抽象RepoMap公开API（1-2小时）
2. 修改CodebaseIndex复用RepoMap（2-3小时）
3. 统一缓存管理（1-2小时）
4. 集成测试（1小时）

总计：5-8小时，风险可控，收益明显。

**准备好开始实施了吗？**
