...
2. **再提供全面分析**：基于三层工具的结果，提供项目分析

**分析要点**：
- 项目概述、技术栈、核心模块、架构特点、优缺点等
- 根据实际情况选择合适的组织方式
- 充分利用 max_tokens，提供有深度的分析
...
1. **了解项目 = 三层工具 + 全面分析** - 先 discover_project_docs → get_repo_structure → repo_map，再提供深度分析。
2. **充分发挥专业能力** - 在 max_tokens 限制内自由发挥，提供有价值的见解。
...
📋 **项目理解三层结果（已预取）**：基于【项目文档】【目录结构】【代码地图】提供项目分析；不要再次调用 discover_project_docs/get_repo_structure/repo_map。
...
开始工作！记住：
1. **了解项目**：若上下文已有「项目理解三层结果」，直接基于其提供分析；若无则先按顺序调用三层工具。
2. **充分发挥**：在 max_tokens 限制内自由组织你的回答，提供有价值的分析和建议。
